...
const getGradientColors = (percentage: number): string => {
  if (percentage > 75) {
    // Red gradient
    return "linear-gradient(to right, #450a0a, #ef4444)";
  } else if (percentage > 50) {
    // Yellow/amber gradient
    return "linear-gradient(to right, #451a03, #f59e0b)";
  } else {
    // Green gradient
    return "linear-gradient(to right, #052e16, #22c55e)";
  }
};

const UsageBar = ({ percentage: propPercentage, segments = 6 }: UsageBarProps) => {
  ...
  return (
    <div className="relative max-h-2 h-2 mt-auto">
      {/* Gray background */}
      <div className={cn("absolute inset-0 max-h-2", "bg-zinc-700")} />
